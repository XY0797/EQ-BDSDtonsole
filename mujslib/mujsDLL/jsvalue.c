...
void js_newcppfunctionx(js_State* J, js_CPPFunction cppfun, const char* name, int length, void* data, js_Finalize finalize)
{
	js_Object* obj = jsV_newobject(J, JS_CCFUNCTION, J->Function_prototype);
	obj->u.c.name = name;
	obj->u.c.CPPfunction = cppfun;
	obj->u.c.constructor = NULL;
	obj->u.c.length = length;
	obj->u.c.data = data;
	obj->u.c.finalize = finalize;
...
void js_newcppfunction(js_State* J, js_CPPFunction cfun, const char* name, int length)
{
	js_newcppfunctionx(J, cfun, name, length, NULL, NULL);
}
