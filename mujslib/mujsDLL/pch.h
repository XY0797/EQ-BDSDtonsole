...
extern "C" {
	__declspec(dllexport) js_State* WINAPI CreatNewJsObj(); //创建js对象，返回指针

	__declspec(dllexport) void WINAPI DestroyJsObj(js_State* jsObjPointer); //销毁js对象

	__declspec(dllexport) void WINAPI RegFunction(js_State* jsObjPointer, js_CPPFunction FunctionPointer, const char* FunctionNameInJs, long ArgumentNumber); //注册js函数，函数指针必须有一个参数接收js对象
	...
	__declspec(dllexport) void WINAPI js_returnlString(js_State* J, const char* v, int n); //函数返回易语言文本型，注意指定长度
	...
	__declspec(dllexport) void WINAPI ExecuteJsCode(js_State* jsObjPointer, const char* JsCode); //执行js代码
}